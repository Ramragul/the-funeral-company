...
useEffect(() => {
  ...
  .then((res) => {
    if (res.data.success) {
      setHeader(res.data.header || null);   // ⬅️ NEW
      ...
      setTasks(formatted);
      setBookingCity(res.data.header?.city || "");
    } else setTasks([]);
  })
  .catch(() => setTasks([]));
  ...
  return (
    <Box p={{ base: 4, md: 8 }}>
      ...
      {header && (
        <Card mb={8} borderRadius="xl" shadow="md" border="1px solid #d9d9d9">
          <CardHeader pb={0}>
            <Heading size="md" color="blue.700">
              Order Details
            </Heading>
            <Text fontSize="sm" color="gray.600">
              Order Date: {new Date(header.order_created_at).toLocaleString()}
            </Text>
          </CardHeader>
          ...
          <CardBody>
            <SimpleGrid columns={{ base: 1, md: 2 }} spacing={4}>
              ...
              <Box>
                ...
                <Text>{header.customer_name} ({header.customer_phone})</Text>
              </Box>

              <Box>
                <Text fontWeight="600">Service Date</Text>
                <Text>{new Date(header.service_date).toDateString()}</Text>
              </Box>

              <Box>
                <Text fontWeight="600">Address</Text>
                <Text>{header.address}, {header.city} - {header.pincode}</Text>
              </Box>

              <Box>
                <Text fontWeight="600">Package</Text>
                <Text>{header.serviceName} — {header.package_variant}</Text>
              </Box>

              <Box>
                <Text fontWeight="600">Total Price</Text>
                <Text>₹{header.total_price}</Text>
              </Box>

              <Box>
                <Text fontWeight="600">Payment Status</Text>
                <Text color={header.payment_status === "paid" ? "green.600" : "red.600"}>
                  {header.payment_status}
                </Text>
              </Box>

              <Box>
                <Text fontWeight="600">Order Status</Text>
                <Text>{header.order_status}</Text>
              </Box>
